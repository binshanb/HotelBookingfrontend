...
//     const authToken = useSelector((state) => state.auth.userInfo?.access);
//     const isNewMessageNotificationShown = useRef(false);
...
//           // Cleanup function to reset isNewMessageNotificationShown when the component unmounts
//           return () => {
//             // setIsNewMessageNotificationShown(false);
//             if (socket) {
//               socket.close();
//             }
//           };
//         }, [decodedAdminInfo, decodedUserInfo]);
...
//             // Check if the received message is for the currently logged-in user
//           // if (data.receiver_id === decodedUserInfo.user_id) {
//           //   // Notify the user
//           //   handleNewMessage();
//           // }
//           };
...
//               instance.get(`/api/chat/chat-messages/?room_id=${roomId}`)
//                 .then(response => {
//                   setChats(response.data);
//                   // handleNewMessageOnce(); 
//                 })
//                 .catch(error => {
...
//     const receiverId = decodedAdminInfo.user_id

//     const roomId = `admin_${decodedAdminInfo.user_id}_user_${decodedUserInfo.user_id}`;
  
//     const messageData = {
//       sender_id: senderId,
//       receiver_id: receiverId, // Assuming you have receiverId defined somewhere
...
//   if (messageInput) {
//     instance.post(`/api/chat/chat-messages/${senderId}/`, messageData)
//       .then(response => {
//         // Check if the response status is a success status (2xx)
//         if (response.status >= 200 && response.status < 300) {
//           // After successfully sending the message, update the local state
...
//         // Optionally, check the error response and provide more specific feedback
...
//     showNotification('User', 'You have a new message');
//     console.log("notification useffect")
//   }, []);
...
//   {chats.map((msg, index) => (
//     <div key={index} className={`flex ${msg.sender === 3  || msg.sender === 1? 'justify-center':'justify-end'} items-center space-x-2 `}>
//       <Avatar sx={{ m: 1, bgcolor: 'secondary.main', width: '20px', height: '20px'}}> 
//         {/* Avatar content */}
//       </Avatar>
//       <div className={`p-2 max-w-xs rounded-lg ${msg.sender === 3 || msg.sender === 1 ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'}`}>
    
//         <p className="m-0">{msg.message}</p>
//         {!msg.is_read && msg.sender === 3 || msg.sender === 1 &&  (
//     <button
//         className="bg-gray-500 hover:bg-gray-200 text-white font-bold py-1 px-2 rounded mt-2"
//         onClick={() => markMessageAsSeen(msg.id)}
//     >
//         {msg.sender === 1 || msg.sender === 3 && (
...
//               {/* {!msg.is_read && msg.receiver_id === decodedUserInfo.user_id && (
//                 <span className="text-xs text-gray-500">Unseen</span>
//               )} */}
//       {/* <IconButton onClick={MarkAllMessagesAsSeen} aria-label="Mark as Seen">
//       <MarkAsSeenIcon />
//     </IconButton> */}
//       </div>
//     </div>
